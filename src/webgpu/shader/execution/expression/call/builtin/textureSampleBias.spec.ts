...
- TODO: test with different derivatives

  Currently, the derivatives are always 1 texel. What we should do is decide on a bias we want to test,
  say 17 (bias is supposed to clamped to 15.99). We only have 3 mip levels so we choose a derivative
  that would produce a mip level of -16 which means our bias of 17 should sample mip level 1.
...
`
  ...
  .fn(async t => {
    ...
    }).map(({ coords, arrayIndex, bias, offset }) => {
      return {
        builtin: 'textureSampleBias',
        coordType: 'f',
        coords,
